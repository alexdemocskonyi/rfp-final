...
function looksZip(buf: Buffer) { return buf && buf.length >= 2 && buf[0] === 0x50 && buf[1] === 0x4b; }
function decodeEntities(s: string) {
  return s.replace(/&amp;/g,"&").replace(/&lt;/g,"<").replace(/&gt;/g,">").replace(/&quot;/g,'"').replace(/&apos;/g,"'");
}
function docxXmlToText(xml: string) {
  const withParas = xml.replace(/<w:p\b[^>]*>/g, "\n");
  const texts = Array.from(withParas.matchAll(/<w:t\b[^>]*>([\s\S]*?)<\/w:t>/g)).map((m) => decodeEntities(m[1]));
  return texts.join("");
}
...
export async function POST(req: Request) {
  try {
    ...
    if (!r.ok) return NextResponse.json({ ok:false, error:"Blob fetch failed: " + r.status }, { status:500 });
